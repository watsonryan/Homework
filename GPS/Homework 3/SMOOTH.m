...
for i = 1:Ntot
    if i==1
        estusr = olspos(Smooth(1:nSat(i),i),satsXYZ(1:nSat(i),:,i));
%         disp(estusr)
        x_ukf(1,1)=estusr(1);
        x_ukf(2,1)=estusr(2);
        x_ukf(3,1)=estusr(3);
        ...
        K = Pxy/Pyy;
        prz=Smooth(1:nSat(i),i);
        x_ukf(1:5,i) = x_m(1:5,1)+K*((prz)-y_m(1:nSat(i)));
        P = P_m - K*Pyy*K';
...
ERROR_Smooth(:,i)=ENU_UKF-ENU_Truth;
Clock_Smooth = x_ukf(4,:)-truthClockBias;
end

Mean_Error_East_Smooth=mean(ERROR_Smooth(1,:))
Mean_Error_North_Smooth=mean(ERROR_Smooth(2,:))
Mean_Error_UP_Smooth=mean(ERROR_Smooth(3,:))
Mean_Clock_Error_Smooth=mean(Clock_Smooth)
...
ENU_UKF = xyz2enu(x_ukf(1:3,i),nomXYZ(:,i));
ENU_Truth = xyz2enu(truthXYZ(:,i),nomXYZ(:,i));
ERROR_NotSmooth(:,i)=ENU_UKF-ENU_Truth;
Clock_NotSmooth = x_ukf(4,:)-truthClockBias;
end

Clock_NotSmooth(1,1)=0; ERROR_NotSmooth(3,1)=0;
Clock_Smooth(1,1)=0; ERROR_Smooth(3,1)=0;

Mean_Error_East_NotSmooth=mean(ERROR_NotSmooth(1,:))
Mean_Error_North_NotSmooth=mean(ERROR_NotSmooth(2,:))
Mean_Error_UP_NotSmooth=mean(ERROR_NotSmooth(3,:))
Mean_Clock_Error_NotSmooth=mean(Clock_NotSmooth)

hold on
subplot(4,1,1)
plot(time,ERROR_Smooth(1,:),'LineWidth',2, 'Color', [1,0,0])
hold on
plot(time,ERROR_NotSmooth(1,:),'LineWidth',2, 'Color', [0,0,0])
title('East')
axis tight
subplot(4,1,2)
plot(time,ERROR_Smooth(2,:),'LineWidth',2, 'Color', [1,0,0])
hold on
plot(time,ERROR_NotSmooth(2,:),'LineWidth',2, 'Color', [0,0,0])
title('North')
axis tight
hold on
subplot(4,1,3)
plot(time,ERROR_Smooth(3,:),'LineWidth',2, 'Color', [1,0,0])
hold on
plot(time,ERROR_NotSmooth(3,:),'LineWidth',2, 'Color', [0,0,0])
title('UP')
axis tight
hold on
subplot(4,1,4)
plot(time,Clock_Smooth,'LineWidth',2, 'Color', [1,0,0])
hold on
plot(time,Clock_NotSmooth,'LineWidth',2, 'Color', [0,0,0])
title('Clock Bias')
axis tight
