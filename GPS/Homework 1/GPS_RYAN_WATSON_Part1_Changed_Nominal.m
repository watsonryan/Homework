...
for i = 1:Length;
    ...
    if z ==1 
        ...
        Pseudorange(1:nSat(1),1) = prData(1:nSat(1),1);
...
for j=1:nSat(z);
    ...
    if j == 1; 
        Computed_Pseudorange(1,1,z)=norm(Sat_XYZ(1,:,z)-nom_XYZ(:,z)')+clockBiasNom*c;
        Unit_Vector(1,:,z)=(Sat_XYZ(1,:,z)-nom_XYZ(:,z)')/norm(Sat_XYZ(1,:,z)-nom_XYZ(:,z)');
    else 
        y=j+1;
        Computed_Pseudorange(y-1,1,z)=norm(Sat_XYZ(y-1,:,z)-nom_XYZ(:,z)')+clockBiasNom*c;
        Unit_Vector(y-1,:,z)=(Sat_XYZ(y-1,:,z)-nom_XYZ(:,z)')/norm(Sat_XYZ(y-1,:,z)-nom_XYZ(:,z)');
...
XYZ_Estimate(:,z) = nom_XYZ(:,z)+dX(1:3,:,z);
nom_XYZ(:,z+1) = XYZ_Estimate(:,z);
Error_ECEF_1(:,z) = norm(XYZ_Estimate(:,z)-truthXYZ(:,z));
...
Final_Error= Error_ECEF_1(Length);
Clock_Bias_Estimate = clockBiasNom+-1*dX(4);
