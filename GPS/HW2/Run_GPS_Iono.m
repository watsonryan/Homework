...
%% Establishes Constants needed for Iono Delay [s]
A1=5e-9;
A2=8.6599e-08;
A3=50400;
A4=100800;
...
for i=1:2400;
    ...
    llh_TRUTH(i,:) = llh;
    %% Calculates zenith ionospheric delay estimate
    ToD(i)=mod(time(i),86400)+19.45*3600;
    if abs(ToD(i)-A3) < A4/4
        Idz(i)=(A1+A2*cos((2*pi*(ToD(i)-A3))/A4))*c;
    else
        Idz(i)=c*A1;
    end
    %% Calculates elevation angle for each iteration
    for j = 1:nSat(i);
        ...
        %% Calculates Zenith Angle and OF for Iono Delay
        zen(i,j)=pi/2-el(i,j);
        OFi(i,j)=(1-(Re*sin(zen(i,j))/(Re+hi))^2)^(-.5);
        I(i,j)=Idz(i)*OFi(i,j);
    end
    ...
    Iono=I(i,1:nSat(i));
    numSat=nSat(i);
    %pr_OBS = 2.546*prDataP1(1:nSat(i),i)-1.546*prDataP2(1:nSat(i),i);
    pr_OBS = prDataP1(1:nSat(i),i);
    sats_XYZ = satsXYZ(1:nSat(i),1:3,i);
    c = 299792458; % m/s
    [Clock_Bias_Estimate,XYZ_Estimate] = GPS_Trop_Iono(nomXYZ,nomCLOCK,pr_OBS,sats_XYZ,numSat,Trop,Iono);
...
Input=menu('Select the Graph to Display','Ionospheric and Tropospheric ENU Model','Comparison of ENU Models','Ionospheric and Tropospheric Clock Model','Comparison of Clock Models');
...
xlabel('Time [s]');
ylabel('Error in East direction [m]');
legend('Trop Model','Trop and Iono Model');
...
if Input==3
    plot(time,E_clock,'r')
    legend('Clock Error with Trop and Iono Model')
    xlabel('Time [s]')
...
if Input==4
    hold on
    plot(time,E_clock,'r')
    legend('Clock Error with Trop Model','Clock Error with Trop and Iono Model')
    xlabel('Time [s]')
    ylabel('Clock Error [m]')
    shg
end
