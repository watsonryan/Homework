function [Clock_Bias_Estimate] = GPS(nomXYZ,nomCLOCK,pr_OBS,sats_XYZ,numSat)
...
% observation model
H=horzcat(uNom2Sat,ones(numSat,1));
% form the innovation vector
deltaRho=prComputed-pr_OBS;
% find the delta solution
dX=inv(H'*H)*H'*deltaRho;

% the clock bias in dX has a negative sign that must be handled
