...
%   insgps_29_state_2.m
%
%   Same as INSGPS_29_STATE but here we are using a
%   different flight trajectory (see LOAD command below)
%
%   The state vector is:
%    x1 = delta x position
%    x2 = delta y position
...
gpstime = 20000;  % specify the GPS time-of-day for the simulation
...
%         [prvec,adrvec] = genrng(1,tru_pos_xyz,svxyzmat,svid,gpstime,[1 1 0 1 1],[],mpmat);
        ...
        h_unit_vec_mat = hmat2(svxyzenumat,[0 0 0]);
...
title('Tightly-Coupled 29-State INS/GPS Filter')
...
title('Tightly-Coupled 29-State INS/GPS Filter - Euler Angle Errors')
ylabel('error in degrees')
xlabel('time in minutes')
...
title('Tightly-Coupled 29-State INS/GPS Filter - Horizontal Position Error')
...
title('Tightly-Coupled 29-State INS/GPS Filter - Covariance Analysis')
