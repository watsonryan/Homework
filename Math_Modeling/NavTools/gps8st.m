%  gps8st.m
%
%  Example of an 8-State GPS Extended Kalman Filter suitable
%  for low-dynamic users
%
%  The state vector is:
%   x1 = delta x position
%   x2 = delta x velocity
...
%            % noise in meters
I = eye(8);
x_pre = zeros(8,1);   % initial state vector prediction
P_pre = 10*eye(8);    % initial prediction error covariance matrix
...
for i = 1:EndLoop,
    ...
    if i == 1,    % if this is the first data point, simply perform ordinary least squares positioning
        ...
        x_nom=zeros(8,1); x_nom(1)=estusr(1); x_nom(3)=estusr(2);
                          x_nom(5)=estusr(3); x_nom(7)=estusr(4);
    else
        n = length(svid);
        nom_pos = [x_nom(1) x_nom(3) x_nom(5)];
        Hpartial = hmat(svxyzmat,nom_pos);        % calculate the direction cosine matrix elements
        H = zeros(n,8); H(:,1)=Hpartial(:,1); H(:,3)=Hpartial(:,2);
        H(:,5)=Hpartial(:,3); H(:,7)=Hpartial(:,4);
        for j = 1:n,
            pr0 = norm(svxyzmat(j,:) - nom_pos) + x_nom(7);  % form the calculated pseudorange to the nominal position
        ...
        %  estimated delta quantities
             ...
             estusr = [x_tot_est(1) x_tot_est(3) x_tot_est(5) x_tot_est(7)];
             estusr_rate = [x_tot_est(2) x_tot_est(4) x_tot_est(6) x_tot_est(8)];
             xyzvel(i,:) = estusr_rate;
        ...
        x_pre = zeros(8,1);  % again, since this is an EKF, the state prediction is zeros
...
title('GPS 8-State Kalman Estimation Errors')
legend('x','y','z','b',0)
...
ylabel('position error in meters')
xlabel('run time in seconds')
title('ZOOM VIEW: GPS 8-State EKF Estimation Errors')
legend('x','y','z','b',0)
axis([0 150 -3 3])
