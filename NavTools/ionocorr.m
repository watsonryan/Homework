...
%   REVISIONS:  15June2010
%               phiu and lambdau computed based on user position
%               (had previously been erroneously computed based on
%                satellite position)
%	Copyright (c) 2002, 2010 by GPSoft
%
global ALPHA BETA
...
phiu = usrllh(1)/pi;  % user geodetic latitude expressed in semi-circles
phii = phiu + psi*cos(az);
if phii > 0.416, phii = 0.416; end
if phii < -0.416, phii = -0.416; end

lambdau = usrllh(2)/pi;  % user geodetic longitude expressed in semi-circles
