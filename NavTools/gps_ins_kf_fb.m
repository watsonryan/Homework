...
vzbias = 60.0e-6*g;       % 60 micro-g z-accel bias
...
for i = 2:npts,
   ...
   height1 = height2;  height2 = est_height(i,1);
    ...
    % K = P_pre_KF*H'*inv(H*P_pre_KF*H' + R); IN A MATLAB-EFFICIENT MANNER
   ...
   %%%%%%% INTO THE INS POS/VEL/ATT UPDATE ALGORITHM AFTER THE FILTER HAS
   %%%%%%% INITIALLY CONVERGED (THIS IS THE CORRECTION FEEDBACK).  
   %%%%%%% AFTERWARDS, IT ONLY INCORPORATES THE
   %%%%%%% UPDATES ONCE PER MINUTE.  NOTE THAT IT ALSO CHECKS TO MAKE
   %%%%%%% SURE THAT A KALMAN FILTER UPDATE HAS OCCURRED
   if (time(i)>653) && (rem(floor(time(i)),60)==0) && (prev_KF_update==1), 
