...
for i = 2:npts,
   ...
   % BODUPDAT:  Update the body-to-nav direction cosine matrix to account
   % for motion of the body-frame
   est_DCMbn = bodupdat(est_DCMbn,est_dtheta(i-1,1:3));  
   
   % LCLEVUPD:  Compute the direction cosine matrix relating the
   % local-level frame at the current and previous epochs
   ...
   % Update the body-to-nav direction cosine matrix to account for motion
   ...
   del_Vl = C*(est_DCMbn*est_delv_b');  % convert from the body-frame to
   %                                    % the local-level frame

   % NAVUPDAT:  Update the earth-to-Local_Level direction cosine matrix
   ...
   % VELUPDAT:  Compute current velocity
   vtmp = velupdat(vel2,vel1,td12,tdex,del_Vl,...
      omega_el_L,est_DCMel,g_extr,0,tdint);
   
   vel_l(i,:) = vtmp';
   
   % Save key parameters and values that will be used for extrapolation in
   % the next epoch
...
% roll0 = roll;
% yaw0 = yaw;
% save jfk_ist0 est_lat0 est_lon0 est_alpha0 vel_l0 pitch0 roll0 yaw0
