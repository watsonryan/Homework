%
%   insdem16.m
%      Great Circle Path INS Demo with Wander Azimuth (FOUCAULT)
%      Error Sources Set to Zero
...
vel_l(1,:) = [vx2 vy2 0];
%
vel_east(1,1) = vel_l(1,1);            % Since a wander azimuth is implemented,
vel_north(1,1) = vel_l(1,2);           % the local-level axes do NOT stay
%                                      % aligned with east and north for
...
%
vertmech = 2;                             % FOUCAULT VERTICAL MECHANIZATION
%
...
for i = 2:npts,
   ...
   cang = cos(-est_alpha(i));     % We are building up a rotation matrix
   sang = sin(-est_alpha(i));     % here to convert velocity from local-level
   Crotmat = [cang sang 0;        % coordinates to East-North-Up.
             -sang cang 0;
                0 0 1];
  vel_ENU(i,:) = ( Crotmat*vtmp )';
   
end

close all

plot(time/3600,yaw*180/pi,time/3600,est_alpha*180/pi,time/3600,(yaw-est_alpha)*180/pi)
title('New York to Istanbul - FOUCAULT MECHANIZATION')
ylabel('yaw, wander angle and heading (deg)')
xlabel('time in hours')
text(4,20,'YAW')
text(5,115,'HEADING')
text(5.75,-100,'WANDER ANGLE')

figure
plot(time(2:50:2152)/3600,vel_l(2:50:2152,1),...
   time(2:50:2152)/3600,vel_l(2:50:2152,2),...
   time(2:50:2152)/3600,vel_ENU(2:50:2152,1),'+',...
   time(2:50:2152)/3600,vel_ENU(2:50:2152,2),'+')
title('New York to Istanbul - FOUCAULT MECHANIZATION')
ylabel('velocity in m/s')
xlabel('time in hours')
text(2,150,'local-level-x')
text(6.25,190,'local-level-y')
text(0.5,230,'EAST')
text(7.5,-120,'NORTH')
